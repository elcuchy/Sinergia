...
# Función para aplicar configuraciones a un directorio base (.config)
apply_user_configs() {
    local TARGET_DIR="$1"
...
EOF
    ...
    # 2. Configurar Panel único en la parte inferior (P10 = Bottom, eliminando dock superior/inferior)
...
# Aplicar a /etc/skel (para futuros usuarios)
apply_user_configs "/etc/skel/.config"

# Aplicar al usuario actual que ejecutó el script
apply_user_configs "$USER_HOME/.config"
chown -R "$REAL_USER:$REAL_USER" "$USER_HOME/.config"

# Extender tema GTK a aplicaciones QT (KDE/Qt apps) a nivel de sistema
