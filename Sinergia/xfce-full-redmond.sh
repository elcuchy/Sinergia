...
# Compilación limpia y segura de YAY con usuario no-root
BUILD_DIR=$(mktemp -d)
...
# ==========================================
echo "==> Personalizando apariencia (Graphite-Dark, Yaru-MATE, Perfil Redmond, Transparencia)..."
...
EOF
    ...
    # 4. Script de Autostart para aplicar el perfil "Redmond" en el primer inicio de sesión
    sudo tee "$TARGET_DIR/autostart/apply-redmond.desktop" > /dev/null << 'EOF'
[Desktop Entry]
Type=Application
Name=Apply Redmond Panel Layout
Exec=bash -c "sleep 2 && xfce4-panel-profiles load /usr/share/xfce4-panel-profiles/layouts/redmond.tar.bz2 && rm -f ~/.config/autostart/apply-redmond.desktop"
Hidden=false
NoDisplay=true
...
echo " El perfil 'Redmond' se aplicará al iniciar sesión."
